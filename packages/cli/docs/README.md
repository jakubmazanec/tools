**@jakubmazanec/cli** • **Docs**

---

# @jakubmazanec/cli

## Namespaces

- [colors](namespaces/colors/README.md)
- [icons](namespaces/icons/README.md)

## Classes

- [Command](classes/Command.md)
- [Program](classes/Program.md)

## Type Aliases

- [CommandAction](type-aliases/CommandAction.md)
- [CommandArguments](type-aliases/CommandArguments.md)
- [CommandOptions](type-aliases/CommandOptions.md)
- [ErrorDetail](type-aliases/ErrorDetail.md)
- [FailureProps](type-aliases/FailureProps.md)
- [Form](type-aliases/Form.md)
- [FormConfig](type-aliases/FormConfig.md)
- [FormProps](type-aliases/FormProps.md)
- [FormResult](type-aliases/FormResult.md)
- [FormRowConfig](type-aliases/FormRowConfig.md)
- [FormRowsConfig](type-aliases/FormRowsConfig.md)
- [FormValue](type-aliases/FormValue.md)
- [FormValues](type-aliases/FormValues.md)
- [HeaderProps](type-aliases/HeaderProps.md)
- [HelpProps](type-aliases/HelpProps.md)
- [ProgramCreateOptions](type-aliases/ProgramCreateOptions.md)
- [ProgramDetail](type-aliases/ProgramDetail.md)
- [ProgramOptions](type-aliases/ProgramOptions.md)
- [ProgramStreams](type-aliases/ProgramStreams.md)
- [SelectInputItem](type-aliases/SelectInputItem.md)
- [StringFormValue](type-aliases/StringFormValue.md)
- [TextInputProps](type-aliases/TextInputProps.md)

## Variables

- [ProgramError](variables/ProgramError.md)

## Functions

- [createForm](functions/createForm.md)
- [Failure](functions/Failure.md)
- [FormRow](functions/FormRow.md)
- [getErrorDetail](functions/getErrorDetail.md)
- [getProgramDetail](functions/getProgramDetail.md)
- [Header](functions/Header.md)
- [Help](functions/Help.md)
- [SelectInput](functions/SelectInput.md)
- [Spinner](functions/Spinner.md)
- [TextInput](functions/TextInput.md)
- [useProgram](functions/useProgram.md)
