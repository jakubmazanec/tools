**@jakubmazanec/ts-utils**

---

# @jakubmazanec/ts-utils

## Classes

- [AssertionError](classes/AssertionError.md)

## Type Aliases

- [AtLeastOneRequired](type-aliases/AtLeastOneRequired.md)
- [DeepPartial](type-aliases/DeepPartial.md)
- [DeepReadonly](type-aliases/DeepReadonly.md)
- [If](type-aliases/If.md)
- [IsEqual](type-aliases/IsEqual.md)
- [LiteralUnion](type-aliases/LiteralUnion.md)
- [NonArray](type-aliases/NonArray.md)
- [NonArrayLike](type-aliases/NonArrayLike.md)
- [OptionalKeys](type-aliases/OptionalKeys.md)
- [Parameters](type-aliases/Parameters.md)
- [Primitive](type-aliases/Primitive.md)
- [RequiredKeys](type-aliases/RequiredKeys.md)
- [ReturnType](type-aliases/ReturnType.md)
- [Simplify](type-aliases/Simplify.md)
- [TupleToUnion](type-aliases/TupleToUnion.md)
- [UnionToIntersection](type-aliases/UnionToIntersection.md)
- [Writable](type-aliases/Writable.md)

## Functions

- [assert](functions/assert.md)
- [is](functions/is.md)
- [isNonNullable](functions/isNonNullable.md)
- [tuple](functions/tuple.md)
