...
async function checkCarsonTemplatesPackage(dependencyBumps: DependencyBumps) {
  ...
  for (let [, projectDependencyBumps] of dependencyBumps) {
    ...
    for (let [dependencyName, bump] of projectDependencyBumps) {
      ...
      if (result?.[0]) {
        let newVersion = semver.minVersion(bump)!;
        let oldVersion = semver.minVersion(result[0])!;
        let newMajorVersion = semver.major(newVersion);
        let oldMajorVersion = semver.major(oldVersion);
        let newMinorVersion = semver.minor(newVersion);
        let oldMinorVersion = semver.minor(oldVersion);

        if (
          newMajorVersion > oldMajorVersion ||
          (oldMajorVersion === 0 && newMinorVersion > oldMinorVersion)
        ) {
          throw new Error(
...
/**
 * A function that checks if Carson template files were changed, which necessitates manual update of the Carson template package.
 */
function checkCarsonTemplateFiles(changedFiles: string[]) {
  // eslint-disable-next-line no-constant-condition, @typescript-eslint/no-unnecessary-condition -- this is autogenerated
  if (false) {
    return;
  }

  if (changedFiles.some((file) => file.startsWith('packages/carson-templates/templates/'))) {
    throw new Error(
      'Bumping version in Carson template files, update of the Carson template package is needed!',
    );
  }
}
...
if (dependencyBumps.get(WORKSPACE_PLACEHOLDER_NAME)?.has(TEMPLATE_PACKAGE_NAME)) {
  ...
  checkCarsonTemplateFiles(diff.split('\n'));
} else {
